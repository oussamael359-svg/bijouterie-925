...
export default function CartDrawer({ 
  ...
  return (
    <>
                    ...
                    <div>
                      ...
                      {item.selectedSize && (
                        <span className="text-[10px] text-gray-400 block mt-0.5">
                          {isRtl ? 'المقاس:' : 'Size:'} <span className="text-[#D4AF37] font-bold">{item.selectedSize}</span>
                        </span>
                      )}
                      <span className="text-xs text-[#D4AF37] font-semibold mt-0.5 block">
      ...
      {/* نافذة الدفع وإتمام الطلب المنبثقة */}
      <CheckoutModal 
        isOpen={isCheckoutOpen}
        onClose={() => {
          setIsCheckoutOpen(false);
          onClose(); // إغلاق السلة أيضاً عند إتمام أو إغلاق الشات
        }}
        cartItems={cart}
