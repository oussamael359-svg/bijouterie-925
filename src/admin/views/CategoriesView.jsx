...
export default function CategoriesView({ categories, setCategories, products, currentLang }) {
  ...
  const handleOpenAdd = () => {
    ...
    setFormData({ id: '', titleAr: '', titleEn: '', descAr: '', descEn: '' });
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            {isRtl ? 'تحكم في أقسام المتجر وعرض المنتجات التابعة لكل قسم.' : 'Manage store categories and sections.'}
          </p>
        </div>
        <button
          ...
          <table className="w-full text-right" dir={isRtl ? 'rtl' : 'ltr'}>
            <thead className="bg-black/40 border-b border-white/10 text-[11px] text-[#D4AF37] uppercase tracking-wider">
              <tr>
                ...
                <th className="p-4">{isRtl ? 'الوصف' : 'Description'}</th>
                ...
                return (
                  <tr key={cat.id} className="hover:bg-white/[0.02] transition">
                    ...
                    <td className="p-4 text-gray-400 max-w-xs truncate">
                      {isRtl ? (cat.descAr || cat.desc) : (cat.descEn || cat.desc)}
                    </td>
                    <td className="p-4">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div 
            className="bg-[#121212] border border-[#D4AF37]/40 w-full max-w-lg rounded-sm p-6 shadow-2xl relative text-right"
            dir={isRtl ? 'rtl' : 'ltr'}
          >
